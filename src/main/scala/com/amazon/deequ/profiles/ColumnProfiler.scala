...
object ColumnProfiler {
  ...
  private[this] def extractGenericStatistics(
    ...
    val inferredTypes = results.metricMap
      .filterNot{
        case (AnalyzerId.DataType(column, None), _) => predefinedTypes.contains(column)
        case _ => true
      }
      .collect { case (AnalyzerId.DataType(column, None), metric: HistogramMetric) =>
    ...
    val approximateNumDistincts = results.metricMap
      .collect { case (AnalyzerId.ApproxCountDistinct(column, None), metric: DoubleMetric) =>
        column -> metric.value.get.toLong
      }

    val completenesses = results.metricMap
      .collect { case (AnalyzerId.Completeness(column, None), metric: DoubleMetric) =>
  ...
  private[this] def extractNumericStatistics(results: AnalyzerContext): NumericColumnStatistics = {
    ...
    val means = results.metricMap
      .collect { case (AnalyzerId.Mean(column, None), metric: DoubleMetric) =>
        metric.value match {
    ...
    val stdDevs = results.metricMap
      .collect { case (AnalyzerId.StandardDeviation(column, None), metric: DoubleMetric) =>
        metric.value match {
          case Success(metricValue) => Some(column -> metricValue)
          case _ => None
        }
    ...
    val maxima = results.metricMap
      .collect { case (AnalyzerId.Maximum(column, None), metric: DoubleMetric) =>
        metric.value match {
          case Success(metricValue) => Some(column -> metricValue)
          case _ => None
        }
    ...
    val minima = results.metricMap
      .collect { case (AnalyzerId.Minimum(column, None), metric: DoubleMetric) =>
        metric.value match {
          case Success(metricValue) => Some(column -> metricValue)
          case _ => None
        }
    ...
    val sums = results.metricMap
      .collect { case (AnalyzerId.Sum(column, None), metric: DoubleMetric) =>
        metric.value match {
          case Success(metricValue) => Some(column -> metricValue)
          case _ => None
        }
